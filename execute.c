...
/*   Updated: 2025/06/29 12:39:55 by anel-men         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	excute_builting(t_cmd **command, t_env **env_list)
{
	...
	else if (ft_strncmp("unset", cmd->args[0], 5) == 0)
		status = ft_unset(env_list, cmd->args + 1);
	else if (ft_strncmp("echo", cmd->args[0], 4) == 0)
		status = echo(cmd->args);
	else if (ft_strncmp("pwd", cmd->args[0], 3) == 0)
		status = pwd(cmd);
	else if (ft_strncmp("cd", cmd->args[0], 2) == 0)
